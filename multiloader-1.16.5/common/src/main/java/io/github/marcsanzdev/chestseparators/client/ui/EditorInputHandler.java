            ...
            if (session.currentState == EditorState.EDIT_FILTER) {
                ...
                if (session.isDraggingListScroll) {
                    ...
                    for (String id : session.currentAllowedItems) {
                        Item item = Registry.ITEM.get(ResourceLocation.tryParse(id));
                        if (item != null
                                && (wlSearch.isEmpty()
                                        || item.getDescription()
                                                .getString()
                    ...
                    if (session.currentState == EditorState.VIEW_GROUPS) {
                        if (ChestConfigManager.getInstance().hasWhitelistClipboardData()) {
                            ...
                            if (Minecraft.getInstance().player != null
                                ...
                                ChestMenu genericHandler = (ChestMenu) Minecraft.getInstance().player.containerMenu;
                        ...
                        if (ChestConfigManager.getInstance().hasClipboardData()) {
                            ...
                            editor.saveSmart();
                            editor.showStatus(
                                    new net.minecraft.network.chat.TranslatableComponent("message.chestseparators.layout_pasted"), ChatFormatting.GREEN);
                            editor.playClickSound(1.0f);
                        } else {
                            editor.playClickSound(0.5f);
                        }
