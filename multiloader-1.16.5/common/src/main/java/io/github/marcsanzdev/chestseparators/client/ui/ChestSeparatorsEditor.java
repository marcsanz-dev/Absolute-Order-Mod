...
public class ChestSeparatorsEditor {
                ...
                () -> {
                    ...
                    if (isFilterState) {
                        ...
                        if (session.currentChestPos != null && !session.isEntityChest && !session.isShulkerBox) {
                            if (me.shedaniel.architectury.networking.NetworkManager.canServerReceive(
    ...
    public static int realIndex(Slot slot) {
        if (slot instanceof io.github.marcsanzdev.chestseparators.mixin.client.CreativeSlotAccessor) {
            io.github.marcsanzdev.chestseparators.mixin.client.CreativeSlotAccessor wrapper =
                    (io.github.marcsanzdev.chestseparators.mixin.client.CreativeSlotAccessor) slot;
            Slot inner = wrapper.chestseparators$getWrappedSlot();
            if (inner != null) return io.github.marcsanzdev.chestseparators.util.SlotIndex.of(inner);
        }
    ...
    public void ensureCreativeInventoryTab() {
        if (this.screen instanceof net.minecraft.client.gui.screens.inventory.CreativeModeInventoryScreen) {
            net.minecraft.client.gui.screens.inventory.CreativeModeInventoryScreen cis =
                    (net.minecraft.client.gui.screens.inventory.CreativeModeInventoryScreen) this.screen;
            if (cis.getSelectedTab() != net.minecraft.world.item.CreativeModeTab.TAB_INVENTORY.getId()) {
                        ...
                        if (!session.filteredItems.contains(item)
                            ...
                            if (isSearching) {
                                ...
                                if (lowerQuery.startsWith("#")) {
                                    ...
                                    matches = net.minecraft.tags.ItemTags.getAllTags().getMatchingTags(item)
                                            .stream()
                                            .anyMatch(rl -> rl.getPath().contains(searchTag));
                                } else {
        ...
        for (String id : session.currentAllowedItems) {
            ...
            if (item != null) {
                ...
                if (wlSearch.isEmpty()) {
                    ...
                    matches = net.minecraft.tags.ItemTags.getAllTags().getMatchingTags(item).stream()
                            .anyMatch(rl -> rl.getPath().contains(searchTag));
                } else {
    ...
    public void syncClientInventoryWhitelists(Map<Integer, SlotWhitelist> whitelists) {
        ...
        for (Slot slot : handler.slots) {
            ...
            if (slot.container instanceof IWhitelistProvider) {
                IWhitelistProvider provider = (IWhitelistProvider) slot.container;
    ...
    public void executeDeposit(boolean shift) {
        ...
        if (!net.minecraft.client.Minecraft.getInstance().player.inventory.getCarried().isEmpty()) return;
        ...
        for (net.minecraft.world.inventory.Slot chestSlot : accessor.getHandler().slots) {
            ...
            if (checkExisting) {
                ...
                if (!net.minecraft.world.item.ItemStack.isSame(stack, chestSlot.getItem())
                        || !io.github.marcsanzdev.chestseparators.util.ItemMatch.sameItemSameTags(stack, chestSlot.getItem()))
                    continue;
    ...
    private int placeIntoPlayerPreview(
        ...
        if (!shift) {
            ...
            for (net.minecraft.world.inventory.Slot ps : emptyFiltered) {
                ...
                previewTargetIncoming.put(ps.index, copyWithCount(stack, add));
                left -= add;
            }
        } else {
    ...
    private void applyPreviewReorder(
        ...
        for (Integer slotId : new java.util.ArrayList<>(previewTargetIncoming.keySet())) {
            ...
            if (!s.getItem().isEmpty() && io.github.marcsanzdev.chestseparators.util.ItemMatch.sameItemSameTags(s.getItem(), inc)) {
