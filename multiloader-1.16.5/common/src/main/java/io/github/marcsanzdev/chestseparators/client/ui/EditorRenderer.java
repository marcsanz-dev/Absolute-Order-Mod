...
public class EditorRenderer {
    ...
    private void renderMagnifierHint(GuiGraphics context) {
        ...
        net.minecraft.network.chat.Component hint = new net.minecraft.network.chat.TranslatableComponent(hintKey, key);
        net.minecraft.client.gui.Font tr = Minecraft.getInstance().font;
    ...
    void renderTransferPreviewOverlay(GuiGraphics context, int mouseX, int mouseY) {
        ...
        if (GlobalChestConfig.instance.showDepositButton
            ...
            editor.depositButton.tooltipText = shift
                    ? new net.minecraft.network.chat.TranslatableComponent("tooltip.chestseparators.push_to_chest.shift")
                            .getString()
                    : new net.minecraft.network.chat.TranslatableComponent("tooltip.chestseparators.push_to_chest").getString();
    ...
    private void drawDurabilityBar(GuiGraphics context, ItemStack stack, int x, int y) {
        ...
        if (stack.isDamaged()) {
            int step = Math.round(13.0F - (float) stack.getDamageValue() * 13.0F / (float) stack.getMaxDamage());
            float remaining = Math.max(
                    0.0F,
                    ((float) stack.getMaxDamage() - (float) stack.getDamageValue()) / (float) stack.getMaxDamage());
            int color = net.minecraft.util.Mth.hsvToRgb(remaining / 3.0F, 1.0F, 1.0F);
            int barX = x + 2;
            int barY = y + 13;
    ...
    private void drawProjectedCount(GuiGraphics context, int x, int y, int count, int color) {
        ...
        context.drawString(Minecraft.getInstance().font, text, x + 17 - textW, y + 9, color, true);
        context.pose().popPose();
    }
