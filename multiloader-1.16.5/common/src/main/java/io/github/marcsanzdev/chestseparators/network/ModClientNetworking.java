 ...
 * via {@code context.queue(...)}. On 1.20.1 the raw-buffer API routes by {@code ResourceLocation}: the
 * payload is parsed from the buffer with its static {@code read(...)}, and the server guards every send
 * with {@code canPlayerReceive}.
 */
public class ModClientNetworking {
    ...
    public static void register() {
        ...
        NetworkManager.registerReceiver(NetworkManager.Side.S2C, ShulkerUUIDPayload.ID, (buf, context) -> {
        ...
        NetworkManager.registerReceiver(NetworkManager.Side.S2C, EditorLockResponsePayload.ID, (buf, context) -> {
            ...
            context.queue(() -> {
                ...
                if (editor != null
                    ...
                    if (payload.granted()) {
                        ...
                        editor.showStatus(
                                new net.minecraft.network.chat.TranslatableComponent("message.chestseparators.locked"), ChatFormatting.RED);
        ...
        NetworkManager.registerReceiver(NetworkManager.Side.S2C, AutoDepositResultPayload.ID, (buf, context) -> {
            AutoDepositResultPayload payload = AutoDepositResultPayload.read(buf);
            context.queue(() -> AutoDepositAnimator.addFlights(payload.flights(), payload.reverse()));
        });
        ...
        NetworkManager.registerReceiver(NetworkManager.Side.S2C, WhitelistS2CPayload.ID, (buf, context) -> {
            ...
            context.queue(() -> {
                ...
                if (player.containerMenu instanceof ChestMenu) {
                    ChestMenu genericHandler = (ChestMenu) player.containerMenu;
                    if (genericHandler.getContainer() instanceof IWhitelistProvider) {
                        IWhitelistProvider provider = (IWhitelistProvider) genericHandler.getContainer();
                        provider.setWhitelists(payload.whitelists());
                    }
                }
            });
        ...
        NetworkManager.registerReceiver(NetworkManager.Side.S2C, EntityWhitelistS2CPayload.ID, (buf, context) -> {
