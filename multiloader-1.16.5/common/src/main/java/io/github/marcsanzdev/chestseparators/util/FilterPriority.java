...
public final class FilterPriority {
    ...
    private static int occupancyPreference(Slot slot, String itemId) {
        ...
        if (whitelists == null) return Integer.MAX_VALUE;
        SlotWhitelist whitelist = whitelists.get(io.github.marcsanzdev.chestseparators.util.SlotIndex.of(slot));
        if (whitelist == null) return Integer.MAX_VALUE;
        int itemRank = whitelist.allowedItems().indexOf(itemId);
        if (itemRank < 0) return Integer.MAX_VALUE;
        ...
        for (Map.Entry<Integer, SlotWhitelist> entry : whitelists.entrySet()) {
            ...
            if (idx < io.github.marcsanzdev.chestseparators.util.SlotIndex.of(slot)) position++;
            if (idx == io.github.marcsanzdev.chestseparators.util.SlotIndex.of(slot)) continue;
    ...
    private static boolean hasRoom(ItemStack existing, ItemStack incoming, int maxCount) {
        ...
        return io.github.marcsanzdev.chestseparators.util.ItemMatch.sameItemSameTags(existing, incoming) && existing.getCount() < maxCount;
    }
     ...
     */
    public static boolean shouldDefer(java.util.List<Slot> slots, int start, int end, Slot candidate, ItemStack stack) {
        if (stack.isEmpty()) return false;
        String itemId = Registry.ITEM.getKey(stack.getItem()).toString();

        boolean candidateIsDedicated = isDedicatedTo(candidate.container, io.github.marcsanzdev.chestseparators.util.SlotIndex.of(candidate), itemId);
        ...
        for (int i = from; i < to; i++) {
            ...
            if (!isDedicatedTo(other.container, io.github.marcsanzdev.chestseparators.util.SlotIndex.of(other), itemId)) continue;
