...
final class FilterClickHandler {
    ...
    boolean onMouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (mouseX >= layout.gridX
            ...
            if (itemIndex >= 0 && itemIndex < session.filteredItems.size()) {
                ...
                if (button == 2) {
                    List<net.minecraft.resources.ResourceLocation> candidates = new ArrayList<>();
                    // 1.16.5 has no TagKey/Holder tag API: query the item's tags via the item-tag collection.
                    for (net.minecraft.resources.ResourceLocation tagId :
                            net.minecraft.tags.ItemTags.getAllTags().getMatchingTags(item)) {
                        String t = tagId.getPath();
     ...
     */
    private static boolean hasMoreThanOneItem(net.minecraft.resources.ResourceLocation tagId) {
        net.minecraft.tags.Tag<Item> tag = net.minecraft.tags.ItemTags.getAllTags().getTag(tagId);
        return tag != null && tag.getValues().size() > 1;
    }
     ...
     */
    private static List<String> dropEquivalentTags(List<net.minecraft.resources.ResourceLocation> candidates) {
        java.util.Map<List<String>, String> byContents = new java.util.LinkedHashMap<>();
        for (net.minecraft.resources.ResourceLocation tagId : candidates) {
            List<String> contents = new ArrayList<>();
            net.minecraft.tags.Tag<Item> tag = net.minecraft.tags.ItemTags.getAllTags().getTag(tagId);
            if (tag != null) {
                for (Item entry : tag.getValues()) {
                    contents.add(Registry.ITEM.getKey(entry).toString());
                }
            }
