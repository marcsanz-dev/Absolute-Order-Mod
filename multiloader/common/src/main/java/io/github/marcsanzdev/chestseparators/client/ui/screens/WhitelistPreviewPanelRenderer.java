    ...
    void renderWhitelistPreviewPanel(GuiGraphics context, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < visibleCount; i++) {
            ...
            if (editor.isHovering(listX + 8, itemY, 16, 16, mouseX, mouseY)
                ...
                context.setTooltipForNextFrame(Minecraft.getInstance().font, item.getName(), mouseX, mouseY);
            }
        }
    }

    private void drawRuleTab(GuiGraphics context, int x, int y, net.minecraft.resources.Identifier icon, boolean isActive) {
        int w = 20;
        int h = 20;
