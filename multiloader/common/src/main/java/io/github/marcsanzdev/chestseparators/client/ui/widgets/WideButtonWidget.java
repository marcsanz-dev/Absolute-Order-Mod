...
public class WideButtonWidget extends CustomWidget {
    ...
    public void render(GuiGraphics context, int mouseX, int mouseY, float delta) {
        ...
        context.drawString(Minecraft.getInstance().font, displayText, 0, 0, iconColor, isDark);
    ...
    private void renderDisabled(GuiGraphics context) {
        ...
        context.pose().translate((float) (x + 22), (float) (y + (height - 9 * scale) / 2));
        context.pose().scale(scale, scale);
        context.drawString(Minecraft.getInstance().font, displayText, 0, 0, disabledColor, isDark);
        context.pose().popMatrix();
    }
}
