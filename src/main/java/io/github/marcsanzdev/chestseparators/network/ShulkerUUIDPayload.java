...
public record ShulkerUUIDPayload(UUID shulkerUUID) implements CustomPayload {
    ...
    public static final CustomPayload.Id<ShulkerUUIDPayload> ID =
            new CustomPayload.Id<>(Identifier.of("chestseparators", "shulker_uuid"));
    ...
    public static final PacketCodec<PacketByteBuf, ShulkerUUIDPayload> CODEC = CustomPayload.codecOf(
            (payload, buf) -> buf.writeUuid(payload.shulkerUUID()), buf -> new ShulkerUUIDPayload(buf.readUuid()));
