...
public final class UiTheme {
    ...
    public static void blitTex(
            GuiGraphics c,
            net.minecraft.util.ResourceLocation atlas,
            int x,
            int y,
            float u,
            float v,
    ...
    public static void tab(GuiGraphics c, int x, int y, int w, int h, boolean hover, boolean active, int attach) {
        // 1.16.5: inactive tab uses the panel's own base colour (0x121218) so tabs and the edit-layout panel
        // read as one surface (on E2 the previous lighter 0x23232C stood out against the glass panel).
        int bg = active ? 0xFF2C6BAE : (hover ? 0xFF2E2E3A : 0xFF121218);
