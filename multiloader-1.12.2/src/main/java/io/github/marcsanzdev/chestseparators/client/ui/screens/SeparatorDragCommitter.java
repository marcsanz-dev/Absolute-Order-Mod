    ...
    void commit(double mouseX, double mouseY) {
        ...
        if (session.currentTab == EditorSessionData.TAB_COMBO) {
            ...
            if (session.comboToolMode == 0) { // Combo Area
                ...
                for (Slot s : sel) selPos.add(SeparatorPreviewRenderer.boxKey(s.xPos, s.yPos));
                for (Slot slot : sel) {
                    ...
                    if (explicitEraser || session.isDragModeErasing) {
                        ...
                        if (nonGrid || !selPos.contains(SeparatorPreviewRenderer.boxKey(slot.xPos, slot.yPos - 18)))
                        ...
                        if (nonGrid || !selPos.contains(SeparatorPreviewRenderer.boxKey(slot.xPos - 18, slot.yPos)))
