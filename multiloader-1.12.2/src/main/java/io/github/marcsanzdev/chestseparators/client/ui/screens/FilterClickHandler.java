...
final class FilterClickHandler {
    ...
    boolean onMouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (button == 0
            ...
            return true;
                ...
                && editor.isHovering(editor.searchBox.x, editor.searchBox.y, editor.searchBox.getWidth(), editor.searchBox.height, mouseX, mouseY)) {
        ...
        if (mouseX >= layout.gridX
            ...
            if (itemIndex >= 0 && itemIndex < session.filteredItems.size()) {
                ...
                if (button == 2) {
                    List<net.minecraft.util.ResourceLocation> candidates = new ArrayList<>();
                    // 1.16.5 has no TagKey/Holder tag API: query the item's tags via the item-tag collection.
                    // TODO(1.12.2 port): item-tag system does not exist before 1.13; there are no tag groups to
                    // gather, so the middle-click "add tag group" shortcut degrades to "no valid groups".
                    // (1.16.5 iterated ItemTags.getAllTags().getMatchingTags(item) and filtered by hasMoreThanOneItem.)
                    for (net.minecraft.util.ResourceLocation tagId :
                            java.util.Collections.<net.minecraft.util.ResourceLocation>emptyList()) {
                        String t = tagId.getResourcePath();
                        if (!t.startsWith("mineable/")
                                && !t.startsWith("needs_")
                                && !t.contains("tools")
                                && !t.startsWith("beacon_")
                                && !t.equals("completes_find_tree_tutorial")
                                && hasMoreThanOneItem(tagId)) candidates.add(tagId);
                    }
                    ...
                    if (!tags.isEmpty()) {
                        ...
                        editor.showStatus(
                                new net.minecraft.util.text.TextComponentTranslation("message.chestseparators.no_valid_groups"), TextFormatting.GRAY);
                } else if (button == 0) {
                    ...
                    session.pendingDragItem = item.getRegistryName().toString();
     ...
     */
    private static boolean hasMoreThanOneItem(net.minecraft.util.ResourceLocation tagId) {
        // TODO(1.12.2 port): item-tag system does not exist before 1.13; a tag's item count cannot be resolved.
        return false;
    }
     ...
     */
    private static List<String> dropEquivalentTags(List<net.minecraft.util.ResourceLocation> candidates) {
        java.util.Map<List<String>, String> byContents = new java.util.LinkedHashMap<>();
        for (net.minecraft.util.ResourceLocation tagId : candidates) {
            List<String> contents = new ArrayList<>();
            // TODO(1.12.2 port): item-tag system does not exist before 1.13; a tag's item contents cannot be listed.
            // (1.16.5 read ItemTags.getAllTags().getTag(tagId).getValues() and mapped each via Registry.ITEM.getKey.)
