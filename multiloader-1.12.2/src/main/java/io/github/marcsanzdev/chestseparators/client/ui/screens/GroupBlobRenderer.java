...
final class GroupBlobRenderer {
    ...
    void renderWhitelistGroups(GuiGraphics context, int mouseX, int mouseY) {
        ...
        if (whitelists != null) {
            ...
            for (Slot slot : editor.accessor.getHandler().inventorySlots) {
                if (!ChestSeparatorsEditor.isEditableSlot(slot)) continue;
                if (editor.isHovering(guiX + slot.xPos - 1, guiY + slot.yPos - 1, 18, 18, mouseX, mouseY)) {
        ...
        if (!activeGreenSlots.isEmpty()) {
            ...
            for (int slotIdx : conflicting) {
                ...
                if (s == null) continue;
                warnSquares.add(new int[] {guiX + s.xPos - 1, guiY + s.yPos - 1, guiX + s.xPos + 17, guiY + s.yPos + 17});
            }
        ...
        if (activeKey != null) {
            ...
            for (Slot slot : editor.accessor.getHandler().inventorySlots) {
                ...
                if (ChestSeparatorsEditor.isPlayerSlot(slot) != activePlayer) {
                    context.fill(
                            guiX + slot.xPos - 1, guiY + slot.yPos - 1, guiX + slot.xPos + 17, guiY + slot.yPos + 17, 0xB0101010);
                }
            }
        }
        ...
        if (!conflicting.isEmpty()) {
            {
                ...
                for (Set<Integer> run : contiguousRuns(conflicting)) {
                    ...
                    for (int slotIdx : run) {
                        ...
                        cx += guiX + slot.xPos + 8f;
                        cy += guiY + slot.yPos + 8f;
                        count++;
                    }
                    if (count == 0) continue;
                    cx /= count;
                    ...
                    if (!iconOnGreen) {
                        ...
                        for (int slotIdx : run) {
                            Slot slot = editor.slotForKey(slotIdx);
                            if (slot == null) continue;
                            float dx = (guiX + slot.xPos + 8f) - cx;
                            float dy = (guiY + slot.yPos + 8f) - cy;
                        ...
                        if (nearest != null) {
                            cx = guiX + nearest.xPos + 8f;
                            cy = guiY + nearest.yPos + 8f;
                        }
                    }
    ...
    private boolean pointOnRun(Set<Integer> run, int guiX, int guiY, float px, float py) {
        for (int slotIdx : run) {
            Slot slot = editor.slotForKey(slotIdx);
            if (slot == null) continue;
            float x0 = guiX + slot.xPos;
            float y0 = guiY + slot.yPos;
    ...
    private List<Set<Integer>> contiguousRuns(Set<Integer> slots) {
        ...
        for (int slotIdx : slots) {
            Slot slot = editor.slotForKey(slotIdx);
            if (slot != null) byPosition.put(posKey(slot.xPos, slot.yPos), slotIdx);
        }
        ...
        while (!remaining.isEmpty()) {
            ...
            while (!queue.isEmpty()) {
                ...
                for (int[] offset : offsets) {
                    Integer neighbour = byPosition.get(posKey(slot.xPos + offset[0], slot.yPos + offset[1]));
    ...
    private static void fillRegion(
        ...
        java.util.TreeSet<Integer> xs = new java.util.TreeSet<>(java.util.Arrays.asList(x0, x1));
        java.util.TreeSet<Integer> ys = new java.util.TreeSet<>(java.util.Arrays.asList(y0, y1));
    ...
    private static Set<Long> occupiedPositions(
        ...
        for (int slotIdx : groupSlots) {
            ...
            if (slot != null && ChestSeparatorsEditor.isEditableSlot(slot)) {
                occupied.add(posKey(slot.xPos, slot.yPos));
            }
        }
        return occupied;
    }
