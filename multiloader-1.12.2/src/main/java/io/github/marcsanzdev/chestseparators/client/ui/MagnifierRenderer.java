...
public final class MagnifierRenderer {
    ...
    public static void render(GuiGraphics context, int cursorX, int cursorY, boolean circle) {
        ...
        // 1.12.2: no Window object — derive the GUI scale + framebuffer size from ScaledResolution and the
        // legacy display dimensions.
        ScaledResolution sr = new ScaledResolution(client);
        double scale = sr.getScaleFactor();
        int fbW = client.displayWidth;
        int fbH = client.displayHeight;
        ...
        // Heap-allocated native buffer: a stack buffer overflows for large loupe sizes. LWJGL2 has no
        // MemoryUtil.memAlloc/memFree, so use a GC-managed direct buffer (BufferUtils) — nothing to free.
        ByteBuffer buf = BufferUtils.createByteBuffer(blockFb * blockFb * 4);
        try {
            ...
            // BufferUtils buffers are garbage-collected; no explicit free is required on E1 (LWJGL2).
        }
