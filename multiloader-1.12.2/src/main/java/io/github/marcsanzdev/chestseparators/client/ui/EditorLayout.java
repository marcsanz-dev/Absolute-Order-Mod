...
public class EditorLayout {
    ...
    public void update(GuiScreen screen, HandledScreenAccessor accessor, int customSidebarYOffset) {
        ...
        // 1.12.2: RecipeBookComponent → GuiRecipeBook. The recipe book accessor is NOT on the interface in
        // this MCP mapping (IRecipeUpdateListener declares only recipesShown(...)); it lives on the concrete
        // GuiInventory as func_194310_f(). Creative has no recipe book, so a GuiInventory check suffices.
        if (screen instanceof net.minecraft.client.gui.inventory.GuiInventory) {
            net.minecraft.client.gui.recipebook.GuiRecipeBook book =
                    ((net.minecraft.client.gui.inventory.GuiInventory) screen).func_194310_f();
            bookOpen = book != null && book.isVisible();
        }
        boolean hasPotion = net.minecraft.client.Minecraft.getMinecraft().player != null
