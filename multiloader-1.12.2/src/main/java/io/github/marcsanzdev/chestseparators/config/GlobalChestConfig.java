...
/**
 * Global (non-per-world) mod settings, persisted as JSON. Identical data shape and semantics to the
 * modern ports — only the config-folder resolution differs: 1.12.2 has no Architectury {@code Platform},
 * so the path comes from Forge's {@code Loader.instance().getConfigDir()} (valid on client and dedicated
 * server alike).
 */
public class GlobalChestConfig {
    ...
    // Resolved lazily rather than in a static initializer so that merely reading config values does not
    // touch the config directory. Loader.getConfigDir() gives the same path on every side.
    private static File configFile() {
        return new File(Loader.instance().getConfigDir(), "chestseparators_config.json");
    }
    ...
    public static class ConfigData {
        ...
        // or off individually from the mod settings; there is no keybind for this.
