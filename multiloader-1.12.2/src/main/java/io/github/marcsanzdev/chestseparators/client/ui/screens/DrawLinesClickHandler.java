                ...
                if (session.currentTab == 0) {
                    if (action != 0 && action != ChestConfigManager.ACTION_BG) {
                        ...
                        if (action == ChestConfigManager.ACTION_TOP || action == ChestConfigManager.ACTION_BOTTOM) {
                            ...
                            session.lockedTraceRowCol = io.github.marcsanzdev.chestseparators.util.SlotIndex.of(slot) / 9;
                            session.lockedLineCoord = (action == ChestConfigManager.ACTION_TOP)
                                    ? (layout.guiY + slot.yPos)
                                    : (layout.guiY + slot.yPos + 16);
                        } else {
                            session.lockedTraceAxis = 2;
                            session.lockedTraceRowCol = io.github.marcsanzdev.chestseparators.util.SlotIndex.of(slot) % 9;
                            session.lockedLineCoord = (action == ChestConfigManager.ACTION_LEFT)
                                    ? (layout.guiX + slot.xPos)
                                    : (layout.guiX + slot.xPos + 16);
                        }
        ...
        for (int i = 0; i < 16; i++) {
            ...
            if (editor.isHovering(x, y, layout.swatchSize, layout.swatchSize, mx, my)) {
                if (button == 2) {
                    ...
                    editor.showStatus(
                            new net.minecraft.util.text.TextComponentTranslation("message.chestseparators.color_copied"), session.copiedColorRGB);
            ...
            if (editor.isHovering(col3X, y, layout.swatchSize, layout.swatchSize, mx, my)) {
                ...
                if (button == 2) {
                    ...
                    if (session.copiedColorRGB != 0) {
                        ...
                        editor.showStatus(new net.minecraft.util.text.TextComponentTranslation("message.chestseparators.color_pasted"), pastedColor);
