...
final class SeparatorPreviewRenderer {
    ...
    void renderHoverPreview(GuiGraphics context, int mouseX, int mouseY) {
        ...
        if (slot != null && ChestSeparatorsEditor.isEditableSlot(slot)) {
            ...
            if (!willErase && tabMode != 2) {
                int existingColor = ChestConfigManager.getInstance().getColor(io.github.marcsanzdev.chestseparators.util.SlotIndex.of(slot), action);
            ...
            int x = layout.guiX + slot.xPos;
            int y = layout.guiY + slot.yPos;
    ...
    void renderDragPreview(GuiGraphics context, int mouseX, int mouseY) {
        ...
        if (tabMode == 1) {
            if (tMode == 0) {
                int startRow = io.github.marcsanzdev.chestseparators.util.SlotIndex.of(session.dragStartSlot) / 9;
                int startCol = io.github.marcsanzdev.chestseparators.util.SlotIndex.of(session.dragStartSlot) % 9;
                int currRow = io.github.marcsanzdev.chestseparators.util.SlotIndex.of(session.dragCurrentSlot) / 9;
                int currCol = io.github.marcsanzdev.chestseparators.util.SlotIndex.of(session.dragCurrentSlot) % 9;
                        ...
                        if (erase
                                && ChestConfigManager.getInstance()
                                                .getColor(
                                                        ...
                                                        ChestConfigManager.ACTION_BG)
                                        == 0) continue;
                        context.fill(guiX + slot.xPos, guiY + slot.yPos, guiX + slot.xPos + 16, guiY + slot.yPos + 16, colorBg);
                    }
                }
            } else {
            ...
            if (tMode == 0) {
                ...
                for (Slot slot : editor.accessor.getHandler().inventorySlots) {
                    ...
                    if (ChestSeparatorsEditor.isPlayerSlot(slot)) continue;
                    int slotIdx = io.github.marcsanzdev.chestseparators.util.SlotIndex.of(slot);
                    int r = slotIdx / 9;
                    int c = slotIdx % 9;
                    if (r >= minRow && r <= maxRow && c >= minCol && c <= maxCol) {
            ...
            if (tMode == 0) {
                ...
                if (geometry.isDraggingRectangle(mouseX, mouseY)) {
                    ...
                    int topY = guiY + startRowSlot.yPos + (yTopExp % 2 == 0 ? -1 : 16);
                    Slot endRowSlot = editor.accessor.getHandler().getSlot((yBotExp / 2) * 9);
                    int botY = guiY + endRowSlot.yPos + (yBotExp % 2 == 0 ? 0 : 17);
                    Slot startColSlot = editor.accessor.getHandler().getSlot(xLeftExp / 2);
                    int leftX = guiX + startColSlot.xPos + (xLeftExp % 2 == 0 ? -1 : 16);
                    Slot endColSlot = editor.accessor.getHandler().getSlot(xRightExp / 2);
                    int rightX = guiX + endColSlot.xPos + (xRightExp % 2 == 0 ? 0 : 17);
    ...
    private void renderPlayerDragPreview(GuiGraphics context, int mouseX, int mouseY) {
        ...
        for (Slot s : sel) selPos.add(boxKey(s.xPos, s.yPos));
        for (Slot slot : sel) {
            int x = guiX + slot.xPos;
            int y = guiY + slot.yPos;
            if (tabMode == 1) {
                ...
                if (nonGrid || !selPos.contains(boxKey(slot.xPos, slot.yPos - 18)))
                    context.fill(x - 1, y - 1, x + 17, y, colorLine);
                if (nonGrid || !selPos.contains(boxKey(slot.xPos, slot.yPos + 18)))
                    context.fill(x - 1, y + 16, x + 17, y + 17, colorLine);
                if (nonGrid || !selPos.contains(boxKey(slot.xPos - 18, slot.yPos)))
                    context.fill(x - 1, y - 1, x, y + 17, colorLine);
                if (nonGrid || !selPos.contains(boxKey(slot.xPos + 18, slot.yPos)))
                    context.fill(x + 16, y - 1, x + 17, y + 17, colorLine);
            }
