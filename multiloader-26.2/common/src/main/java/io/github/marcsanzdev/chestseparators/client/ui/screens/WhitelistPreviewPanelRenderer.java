...
final class WhitelistPreviewPanelRenderer {
    ...
    void renderWhitelistPreviewPanel(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        ...
        if (!session.isPreviewing) {
            ...
            for (net.minecraft.util.FormattedCharSequence line :
                ...
                context.text(Minecraft.getInstance().font, line, textX, textY, 0xFF666666, false);
        ...
        if (session.previewItems.isEmpty()) {
            ...
            for (net.minecraft.util.FormattedCharSequence line :
                ...
                context.text(Minecraft.getInstance().font, line, textX, textY, 0xFF888888, false);
                textY += Minecraft.getInstance().font.lineHeight + 2;
            }
        }
        ...
        for (int i = 0; i < visibleCount; i++) {
            ...
            net.minecraft.world.item.Item item =
                    net.minecraft.core.registries.BuiltInRegistries.ITEM.getValue(net.minecraft.resources.Identifier.tryParse(itemIdStr));
        ...
        for (int i = 0; i < visibleCount; i++) {
            ...
            net.minecraft.world.item.Item item = net.minecraft.core.registries.BuiltInRegistries.ITEM.getValue(
                    net.minecraft.resources.Identifier.tryParse(session.previewItems.get(idx)));
            int itemY = listViewY + (i * 18) - (int) (session.listScrollY % 18) + 2;

            if (editor.isHovering(listX + 8, itemY, 16, 16, mouseX, mouseY)
                    && mouseY >= listViewY
                    && mouseY <= listViewY + listViewH) {
                context.outline(listX + 7, itemY - 1, 18, 18, 0xFFFFFFFF);
                context.setTooltipForNextFrame(Minecraft.getInstance().font, item.getName(item.getDefaultInstance()), mouseX, mouseY);
            }
        }
    }

    private void drawRuleTab(GuiGraphicsExtractor context, int x, int y, net.minecraft.resources.Identifier icon, boolean isActive) {
        int w = 20;
        int h = 20;
        ...
        context.blit(pipeline, icon, x + 2, y + 2, 0.0F, 0.0F, 16, 16, 128, 128, 128, 128, color);
        if (isActive) {
            context.pose().popMatrix();
        }
    }
