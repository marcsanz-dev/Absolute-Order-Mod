...
public class ScreenDrawLines extends AbstractEditorScreen {
    ...
    private void drawPaletteArea(GuiGraphicsExtractor context, int mouseX, int mouseY, int tabMode) {
    ...
    private void drawTabs(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            context.blit(pipeline, icon, tabX + 2, tabY + 2, 0.0F, 0.0F, 16, 16, 128, 128, 128, 128, -1);
            if (isSelected) {
            ...
            if (hover) {
                ...
                context.setTooltipForNextFrame(Minecraft.getInstance().font, name, mouseX, mouseY);
            }
        }
    }
