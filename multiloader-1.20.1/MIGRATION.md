...
**Still to do — remaining WIRING (the editor code + the 5 big client mixins are DONE and both jars build GREEN):**
1. ✅ DONE — Big client mixins (`mixin/client/`), registered in the `client` section of `absoluteorder.mixins.json`:
   **GenericContainerScreenMixin** (over `AbstractContainerScreen`: init / renderContents [TAIL overlay + HEAD recipe-book normalize] / renderSlots / renderTooltip / renderSlotHighlightBack/Front / renderSlot / keyPressed [KeyMapping.matches] / charTyped / removed / mouseScrolled + NEW `mouseClicked/mouseReleased/mouseDragged` injects forwarding to `editor.inputHandler.*`; cursor via `requestCursor(CursorType.DEFAULT)`), **CreativeInventoryScreenMixin** (`selectTab`/`checkTabClicked`/`checkTabHovering`/keyPressed/charTyped), **InGameHudHotbarLinesMixin** (Gui `renderSlot`/`renderItemHotbar`; `getSelectedSlot()`), **StatusEffectsDisplayMixin** (→ `EffectsInInventory.render`), **ArmorEquipClientMixin** (→ `MultiPlayerGameMode.useItem`, `DataComponents.EQUIPPABLE`, `ChestSeparatorsState.INVENTORY_FILTERS`). The 8 client accessors are now registered too.
...
3. `mixin/WorldMixin` (depends on ChestConfigManager) — add to the `client` section too.
...
Client-mixin E5 target names (verified): yarn `HandledScreen.renderMain`→`AbstractContainerScreen.renderContents`; `drawSlots`→`renderSlots`; `drawSlot`→`renderSlot`; `drawMouseoverTooltip`→`renderTooltip`; `drawSlotHighlightBack/Front`→`renderSlotHighlightBack/Front` (private); yarn `GenericContainerScreen`→`ContainerScreen`; `HorseScreen`→`HorseInventoryScreen`; `CreativeInventoryScreen`→`CreativeModeInventoryScreen` (`setSelectedTab`→`selectTab`, `isClickInTab`→`checkTabClicked`, `renderTabTooltipIfHovered`→`checkTabHovering`); `InGameHud`→`Gui` (`renderHotbarItem`→`renderSlot`, `renderHotbar`→`renderItemHotbar`); `StatusEffectsDisplay`→`EffectsInInventory`; `ClientPlayerInteractionManager`→`MultiPlayerGameMode` (`interactItem`→`useItem`); `Element`→`GuiEventListener`, `ClickableWidget`→`AbstractWidget`; `context.setCursor(Cursor.DEFAULT)`→`context.requestCursor(CursorType.DEFAULT)`; `Inventory.selectedSlot`→`getSelectedSlot()`.
...
## Config screen + recipe-viewer compat — status

- **Config screen (Cloth Config): DONE, both loaders.** `client/config/AbsoluteOrderConfigScreen.buildScreen(parent)` (categories client/server/hotkeys, `fillKeybindingField`, custom icon entries). Served via Fabric ModMenu (`ModMenuIntegration`) and NeoForge `IConfigScreenFactory` (`NeoForgeClientBootstrap`). Cloth Config is `modImplementation` on both platforms.
- **EMI: DONE, both loaders.** `compat/ChestSeparatorsEmiPlugin` (`@EmiEntrypoint`, auto-scanned on Fabric + NeoForge) — full-screen exclusion zone while the editor is open. EMI is compile-only.
- **REI: Fabric DONE / NeoForge DEFERRED.** `compat/ChestSeparatorsReiPlugin implements REIClientPlugin` (common) + Fabric `rei_client` entrypoint. NeoForge REI discovers plugins via its own `@REIPluginClient` annotation (a different artifact than the fabric REI api used to compile common). **Deliberately deferred**: there is NO REI build for 1.21.11 (Fabric or NeoForge) — the APIs are compile-only and REI cannot be runtime-tested on this snapshot at all. Degrades gracefully: without the NeoForge annotation, REI-on-NeoForge simply wouldn't apply the exclusion zone. **TODO when a 1.21.11 NeoForge REI exists:** add `modCompileOnly` neoforge REI api to the neoforge module + a thin `@REIPluginClient class NeoForgeReiPlugin extends ChestSeparatorsReiPlugin {}` to reuse the common exclusion-zone logic.
- **JEI: not applicable.** The original mod ships no JEI plugin (only EMI + REI); parity requires none.
