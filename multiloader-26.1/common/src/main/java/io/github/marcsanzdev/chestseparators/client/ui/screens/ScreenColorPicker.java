...
public class ScreenColorPicker extends AbstractEditorScreen {
    ...
    protected void buildWidgets() {
        ...
        // TODO(26.1): EditBox no longer exposes setFilter — input restriction dropped for now (downstream parsing still guards).
    ...
    public void render(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.pose().translate((float) (x + 12), (float) (y + 12 + (9 * (1.0f - titleScale)) / 2));
        context.pose().scale(titleScale, titleScale);
        context.text(client.font, titleText, 0, 0, isDark ? 0xFFE0E0E0 : 0xFF333333, false);
        ...
        context.pose().translate((float) (x + 12), (float) (y + 26) + yOffset);
        context.pose().scale(scale, scale);

        int instructY = 0;
        for (net.minecraft.util.FormattedCharSequence line : wrappedLines) {
            context.text(client.font, line, 0, instructY, isDark ? 0xFFFFFFFF : 0xFF222222, false);
        ...
        context.outline(cursorX - 2, cursorY - 2, 5, 5, 0xFF000000);
        context.outline(cursorX - 1, cursorY - 1, 3, 3, 0xFFFFFFFF);
        ...
        context.text(client.font, "#:", textX, fieldY + 1, dynamicColor, true);
        context.text(client.font, "R:", textX, fieldY + 21, 0xFFFF5555, true);
        context.text(client.font, "G:", textX, fieldY + 37, 0xFF55FF55, true);
        context.text(client.font, "B:", textX, fieldY + 53, 0xFF5555FF, true);

        if (hexField != null) hexField.extractRenderState(context, mouseX, mouseY, delta);
        if (rField != null) rField.extractRenderState(context, mouseX, mouseY, delta);
        if (gField != null) gField.extractRenderState(context, mouseX, mouseY, delta);
        if (bField != null) bField.extractRenderState(context, mouseX, mouseY, delta);
    ...
    private void renderEyedropperCursor(GuiGraphicsExtractor context, int mx, int my) {
