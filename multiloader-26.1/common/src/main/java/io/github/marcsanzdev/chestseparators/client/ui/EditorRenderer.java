...
public class EditorRenderer {
    ...
    public void render(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        if (session.isPresetsMenuOpen) {
            ...
            if (!editor.presetsMenu.isPreviewActive()) {
                ...
                context.pose().translate((float) accessor.getX(), (float) accessor.getY());
     ...
     */
    private void renderToolbar(GuiGraphicsExtractor context, int hoverX, int hoverY, float delta) {
        parkToolbarButtons();
    ...
    private void renderMagnifierHint(GuiGraphicsExtractor context) {
        ...
        context.text(tr, hint, (screen.width - tr.width(hint)) / 2, screen.height - 12, 0x70FFFFFF, false);
    }
     ...
     */
    private void renderUndoHighlights(GuiGraphicsExtractor context) {
    ...
    public void renderSavedLinesLayer(GuiGraphicsExtractor context) {
        if (session.isPresetsMenuOpen) return;
        doRenderSavedLinesLayer(context);
    }

    private void doRenderSavedLinesLayer(GuiGraphicsExtractor context) {
    ...
    private void renderEdgesInPaintOrder(
            GuiGraphicsExtractor context, int x, int y, int slotIndex, int lineAlpha, boolean erasePreview) {
    ...
    private void drawCorner(GuiGraphicsExtractor context, int cx, int cy, int colorA, int seqA, int colorB, int seqB) {
        int color;
    ...
    private void renderStatusMessage(GuiGraphicsExtractor context) {
    ...
    public void renderNormalModeOverlay(GuiGraphicsExtractor context, int mouseX, int mouseY) {
     ...
     */
    void renderTransferPreviewOverlay(GuiGraphicsExtractor context, int mouseX, int mouseY) {
    ...
    private void renderDepositPreview(GuiGraphicsExtractor context) {
     ...
     */
    private void drawGhostItem(GuiGraphicsExtractor context, ItemStack stack, int x, int y, int wash) {
    ...
    private void drawDurabilityBar(GuiGraphicsExtractor context, ItemStack stack, int x, int y) {
    ...
    private void drawProjectedCount(GuiGraphicsExtractor context, int x, int y, int count, int color) {
        String text = String.valueOf(count);
        int textW = Minecraft.getInstance().font.width(text);
        context.text(Minecraft.getInstance().font, text, x + 17 - textW, y + 9, color, true);
    }
     ...
     */
    private void renderMagnifier(GuiGraphicsExtractor context, int mouseX, int mouseY) {
