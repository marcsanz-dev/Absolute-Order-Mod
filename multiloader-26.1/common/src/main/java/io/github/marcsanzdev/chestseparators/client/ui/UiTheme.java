...
public final class UiTheme {
     ...
     */
    public static void roundRect(GuiGraphicsExtractor c, int x, int y, int w, int h, int col) {
    ...
    public static void roundBorder(GuiGraphicsExtractor c, int x, int y, int w, int h, int col) {
    ...
    public static void panel(GuiGraphicsExtractor c, int x, int y, int w, int h) {
     ...
     */
    public static void tab(GuiGraphicsExtractor c, int x, int y, int w, int h, boolean hover, boolean active, int attach) {
     ...
     */
    public static void scrollbar(GuiGraphicsExtractor c, int x, int y, int w, int h, int thumbY, int thumbH) {
        inset(c, x, y, w, h);
        int col = 0x70FFFFFF;
        int tx = x + 1;
        int tw = w - 2;
    ...
    public static void inset(GuiGraphicsExtractor c, int x, int y, int w, int h) {
     ...
     * their aspect ratio. ALWAYS pair with {@code context.pose().popMatrix()}.
     */
    public static void pushActiveContent(GuiGraphicsExtractor c, int x, int y, int w, int h) {
        float s = Math.min((w - 2f) / w, (h - 2f) / h);
        float cx = x + w / 2f;
        float cy = y + h / 2f;
        c.pose().pushMatrix();
        c.pose().translate(cx, cy);
        c.pose().scale(s, s);
        c.pose().translate(-cx, -cy);
    }

    /** Cristal glass button background for the given hover/active state. */
    public static void button(GuiGraphicsExtractor c, int x, int y, int w, int h, boolean hover, boolean active) {
        if (active) {
